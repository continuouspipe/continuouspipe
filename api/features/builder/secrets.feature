Feature:
  ...
  @integration
  Scenario:
    When I send a build request for the image "docker.io/continuouspipepublicrobot/test" from the fixture repository "build-args" with the following environment:
      | name          | value |
      | MY_CUSTOM_ENV | foo   |
    Then the build should be successful
    And the file "/result" in the image "docker.io/continuouspipepublicrobot/test" should contain "foo"
