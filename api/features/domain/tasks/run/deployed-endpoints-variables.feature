Feature:
  ...
  Scenario: If a run task is after a deployment, it should inject the proper underscored public endpoint variables
    Given I have a "continuous-pipe.yml" file in my repository that contains:
    """
    tasks:
        deploy:
    ...
    """
    When a tide is started
    And the service "foo-bar" was created with the public address "1.2.3.4"
    And the deployment succeed
    Then the commands should be run with the following environment variables:
      | name                            | value   |
      | SERVICE_FOO_BAR_PUBLIC_ENDPOINT | 1.2.3.4 |
